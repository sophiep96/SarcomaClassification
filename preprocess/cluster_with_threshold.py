#Adapted from code by Matineh Akhlaghinia. This version uses the mean HED values instead of RGB and introduces a threshold on these values
...
OFFSET_final = 20
CLUSTERS = 3
DESTINATION_PATH = "/home/sophie/Documents/test_view_slide/labelled_tiles_threshold"
DATA_PATH = "/home/sophie/Documents/test_view_slide/tiles"
...
def createHematoxFeature(ihc_hed):
    eu, hema, dab = ihc_hed[:, :, 1], ihc_hed[:, :, 0], ihc_hed[:, :, 2]
    size_i = len(hema)
    num_pixels = 512*512
    mean_hema = mean_eu = mean_dab = 0
    high_thresh_hema = -0.45 #-0.63 to -0.41
    low_thresh_hema = -0.59
    high_thresh_eu = 0.20  #0.13 to 0.24
    low_thresh_eu = 0.17
    above_hema = 0
    below_hema = 0
    above_eu = 0  
    ...
    for i in range(size_i):
        ...
        for j in range(size_j):
            ...
            mean_eu += eu[i][j]
            mean_dab += dab[i][j]
            if hema[i][j] > high_thresh_hema:
                above_hema += 1
            elif hema[i][j] < low_thresh_hema:
                below_hema += 1
            if eu[i][j] > high_thresh_eu:
                above_eu += 1
            elif eu[i][j] < low_thresh_eu:
                below_eu += 1
    return mean_hema/num_pixels, mean_eu/num_pixels, mean_dab/num_pixels, above_hema, below_hema, above_eu, below_eu
...
for file in listdir(DATA_PATH):
    ...
    for f in tqdm(listdir(path_to_tiles)):
        ...
        rgb_mean = createHematoxFeature(ihc_hed)
