...
class ClassSpecificImageGeneration():
    ...
    def __init__(self, model, target_class):
        ...
        #self.created_image = Image.open("SS.jpeg").convert('RGB') #Use this for Deep Dream
        self.created_image = np.uint8(np.random.uniform(0, 255, (299, 299, 3))) #Use this for CSIG
    ...
    def generate(self):
        ...
        for i in range(1, 201):
            ...
            class_loss = -output[0, self.target_class] + output[0, self.target_class]
            print('Iteration:', str(i), 'Loss', "{0:.2f}".format(class_loss.data.numpy()))
            # Zero grads
            self.model.zero_grad()
            # Backward
            class_loss.backward() #class_loss.backward()
            # Update image
            optimizer.step()
            # Recreate image
